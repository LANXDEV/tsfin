...
from tsfin.constants import QUOTES, TENOR_PERIOD, MATURITY_DATE, QUOTE_TYPE
...
class Instrument:
    ...
    def rate_helper(self, date, last_available=True, tenor=None, fixing_days=None, calendar=None,
                    busdayconvention=None, day_counter=None, *args, **kwargs):
        """Try to deduce a helper for yield curve construction.

        Warnings
        --------
        The result may be wrong. Use this method only for emergency/temporary cases. It is safer to instantiate a
        'real' financial instrument class and call its ``rate_helper`` method instead of using this.
        ...
        last_available: bool, optional
            Defaults to True.
        tenor: QuantLib.Period, optional
            Defaults to a call to ``self.tenor(date)``.
        fixing_days: int, optional
            Defaults to ``0``.
        calendar: QuantLib.Calendar, optional
            Defaults to ``QuantLib.UnitedStates(ql.UnitedStates.NYSE)``.
        busdayconvention: int, optional
            Defaults to ``QuantLib.Unadjusted``.
        day_counter: QuantLib.DayCounter, optional
            Defaults to ``QuantLib.Actual365Fixed()``.

        Returns
        -------
        QuantLib.RateHelper-like, None
            RateHelper object for yield curve construction.
        """
        if tenor is None:
            tenor = self.tenor(date)
            if tenor is None:
                return None
        if fixing_days is None:
            fixing_days = 0
        if calendar is None:
            calendar = ql.UnitedStates(ql.UnitedStates.NYSE)
        if busdayconvention is None:
            busdayconvention = ql.Unadjusted
        ...
        rate = ql.InterestRate(rate, day_counter, ql.Compounded, ql.Semiannual).equivalentRate(ql.Simple, ql.Annual,
                                                                                               time).rate()
        return ql.DepositRateHelper(ql.QuoteHandle(ql.SimpleQuote(rate)), tenor, fixing_days, calendar,
                                    busdayconvention, False, day_counter)
