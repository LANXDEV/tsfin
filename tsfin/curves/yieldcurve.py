...
class YieldCurveTimeSeries:
    ...
    def _get_helpers(self, date):
        ...
        if self.keep_only_on_the_run_month:
            ...
            monthed_helpers = dict()
            for maturity_date, helper in helpers.items():
                month_year = self._date_to_month_year(maturity_date)
                existing_helper = monthed_helpers.get(month_year, None)
                if existing_helper is None:
                    monthed_helpers[month_year] = helper
                else:
                    monthed_helpers[month_year] = max((helper, existing_helper),
                                                      key=attrgetter('issue_date'))
            helpers = {ndhelper.maturity_date: ndhelper for ndhelper in monthed_helpers.values()}
    ...
    def update_curves(self, dates):
        ...
        for date in dates:
            ...
            yield_curve = ql.PiecewiseLinearZero(qldate, helpers, self.day_counter)  # Just bootstraping the nodes
